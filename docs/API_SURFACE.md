...
The CLI inventory currently contains 159 user-facing modules. Every module is
...
in `pyproject.toml`. The current registered inventory is 57 scripts.
...
At the M3-1 baseline, conservative coverage is:

- Full Madagascar/alias command surface: `121 / 2114 = 5.72%`.
- Core `system/` + `plot/main`: `108 / 301 = 35.88%`.
