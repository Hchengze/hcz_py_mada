...
- G4-1: Madagascar source architecture reading and implementation planning
  pass completed. It adds `docs/MADAGASCAR_SOURCE_READING.md` and
  `docs/CODE_COMMENT_GUIDE.md` to document Original Madagascar source-tree
...
- M0-3: direct `system/main` array-shape coverage adds source-aligned `sfpad`
  and `sfspray` registrations for `../src-master/system/main/pad.c` and
  `../src-master/system/main/spray.c`. The existing bounded Python APIs gain
  `RSFData.pad(...)` and `RSFData.spray(...)`, plus `pymada-pad` and
  `pymada-spray` console scripts. M0-3 does not add root exports, change
  denominators, continue Forward Modeling/DAS/Localization/solver work, or
  modify Original Madagascar source.
- M1-1: source-aligned `system/generic` migration begins with `sfclip`,
  `sfnoise`, and `sfboxsmooth`, mapped to
  ...
  adds `value=`/non-finite handling to the bounded `sfclip` subset, and adds
...
- M2-4: source-aligned `system/seismic` gather organization / stacking utility
  migration continues with `sfcmp2shot`, `sfintbin`, and `sfintbin3`, mapped to
  `../src-master/system/seismic/Mcmp2shot.c`,
  `../src-master/system/seismic/Mintbin.c`, and
  `../src-master/system/seismic/Mintbin3.c`. The bounded subsets add regular
  CMP-to-shot gather reorganization and numeric integer-header trace sorting
  to 2D/3D bin grids with Python API, RSFData methods, CLI modules, console
...
- M3-4: official source gap second pass adds source-aligned `sfgrad2`,
  `sfgrad3`, and `sflpad`, mapped to
  `../src-master/system/generic/Mgrad2.c`,
  `../src-master/system/generic/Mgrad3.c`, and
  `../src-master/system/generic/Mlpad.c`, with Sobel stencil details drawn from
  `../src-master/api/c/edge.c`. The bounded subsets add Python topic APIs,
  RSFData chain methods, CLI modules, console scripts, tests, docs, and
  coverage mapping. `sfshot2cmp` is deferred for paired geometry validation;
  `sfmodrefl`/`sfmodrefl2`, `sflinsincos`, and `sfricker1` are deferred for
  elastic modeling, angle-grid integration, or different upstream semantics.
  M3-4 does not handle GitHub Actions Windows-only diagnostics, continue
...
- `pymadagascar/generic`: spike/math/window/info/put/attr, file ops, stats,
  ...
  pad/spray/tile, Sobel gradient subsets, lpad interleaving, cat/transp.
