"""CLI for zero-phase frequency-domain RSF lowpass filtering."""
...
from pymadagascar.signal.filter import FilterError, lowpass_rsf


HELP_TEXT = """Lowpass parameters:
...
def lowpass_command(params: RSFParams) -> int:
    ...
    try:
        lowpass_rsf(input_path, output, fcut=fcut, axis=axis, taper=taper)
...
def main(argv: Sequence[str] | None = None) -> int:
    return run_rsf_command(
        ...
        argv,
        prog="pymada-lowpass",
        description="Apply a zero-phase FFT lowpass filter to RSF data.",
        help_text=HELP_TEXT,
    )
