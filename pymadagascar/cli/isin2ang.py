"""CLI for bounded inverse-sine stack-panel to angle transform."""
...
from pymadagascar.seismic.angle import AngleTransformError, isin2ang_rsf


HELP_TEXT = """ISin2Ang parameters:
  input.rsf               Stack panel with an inverse-sine transform axis.
...
def isin2ang_command(params: RSFParams) -> int:
...
def main(argv: Sequence[str] | None = None) -> int:
    return run_rsf_command(
        isin2ang_command,
        argv,
        prog="pymada-isin2ang",
        description="Apply bounded sfisin2ang angle-axis resampling.",
        help_text=HELP_TEXT,
    )
