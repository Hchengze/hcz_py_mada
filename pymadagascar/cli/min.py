"""Madagascar-style CLI for RSF minimum statistics."""
...
from pymadagascar.generic.stats import StatError, format_stat, min_rsf
from pymadagascar.io.rsf import RSFError


HELP_TEXT = """min parameters:
  input.rsf            Input RSF header path.
  axis=0               Global minimum. Use 1-based RSF axis for axis-wise values.
...
def min_command(params: RSFParams) -> str:
...
def main(argv: Sequence[str] | None = None) -> int:
    return run_rsf_command(
        ...
        argv,
        prog="pymada-min",
        description="Report minimum values from an RSF dataset.",
        help_text=HELP_TEXT,
    )
