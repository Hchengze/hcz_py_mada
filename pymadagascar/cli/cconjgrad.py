"""Module-only CLI for complex small-data conjugate-gradient solves."""
...
from pymadagascar.generic.linear_operator import LinearOperatorError, MatrixOperator, complex_conjgrad_solve
from pymadagascar.io.rsf import RSFError, read_rsf, write_rsf


HELP_TEXT = """Cconjgrad parameters:
  matrix.rsf rhs.rsf      2-D complex matrix and right-hand-side RSF.
  out=x.rsf               Output complex solution RSF path.
  mode=normal             normal least-squares mode, or mode=hermitian.
  niter=10 tol=1e-6       Iteration count and convergence tolerance.
  damp=0.0                Tikhonov damping for normal equations or A+damp^2 I.

This is a pure Python matrix-backed subset of sfcconjgrad. It uses Hermitian
adjoints and does not support external command operators or preconditioners.
"""


def cconjgrad_command(params: RSFParams) -> int:
    try:
        ...
        write_rsf(output_path, np.asarray(result.solution, dtype=np.complex64), header=header)
...
def main(argv: Sequence[str] | None = None) -> int:
    return run_rsf_command(
        cconjgrad_command,
        argv,
        prog="python -m pymadagascar.cli.cconjgrad",
        description="Solve a complex small-data matrix system with CG.",
        help_text=HELP_TEXT,
    )
